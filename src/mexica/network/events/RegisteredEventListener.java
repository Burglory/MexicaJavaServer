...
public class RegisteredEventListener {
	...
	private Class<? extends NetworkEvent> getEventClassForMethod(Method m) {
		...
		if (types.length == 1) {
			try {
				NetworkLogger.Log("Returning EventClass: "
						+ types[0].getName());
	...
	private Collection<Method> getAllEventMethods(Class<?> c) {
		...
		for (Method m : c.getDeclaredMethods()) {
			if (m.getAnnotation(NetworkEvents.class) != null) {
				NetworkLogger.Log("Found an ServerEvent Method: "
						+ m.getName());
				result.add(m);
			}
		}
	...
	public void dispatchEvent(NetworkEvent e) {
		...
		for (Method m : map.get(e.getClass())) {
			try {
				NetworkLogger.Log("Dispatching ServerEvent for Method: "
						+ m.getName() + " in Class: "
