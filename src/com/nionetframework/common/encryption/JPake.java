...
/**
 * JPake - For JPake implementation on Android Inspired by JPAKEDemo.java
 * written by Dr. Feng Hao and the J-PAKE model
 * 
 * @see http 
...
public class JPake {
	...
	public void step1(String signerId) throws Exception {
		...
		BigInteger[] sigX1 = generateZeroKnownProofs(p, q, g, gx1, x1, signerId);
		BigInteger[] sigX2 = generateZeroKnownProofs(p, q, g, gx2, x2, signerId);
	...
	public BigInteger[] generateZeroKnownProofs(BigInteger p, BigInteger q,
			BigInteger g, BigInteger gx, BigInteger x, String signerId)
			throws Exception {
	...
	public boolean verifyZeroKnownProof(BigInteger p, BigInteger q,
	...
	public BigInteger getHash(BigInteger g, BigInteger gr, BigInteger gx,
			String signerId) throws NoSuchAlgorithmException {
	 ...
	 * 
	 * @param gx1
	 *            is derived from the sender's step1
	 * @param gx3
	 *            is gx1 of receiver's. Derived from the receiver's step1
	 * @param gx4
	 *            is gx2 of receiver's. Derived from the receiver's step1
	 * @param x2
	 *            is derived from the sender's step1
	 * @param pwd
	 *            is derived from the EditText of the sender's.
	 * @throws Exception
	 */

	public void step2(BigInteger gx1, BigInteger gx3, BigInteger gx4,
			BigInteger x2, BigInteger pwd, String signerId) throws Exception {
		BigInteger gA = gx1.multiply(gx3).multiply(gx4).mod(p);
		// pwd is the shared passwd
		BigInteger A = gA.modPow(x2.multiply(pwd).mod(q), p);
		BigInteger[] sigX2s = generateZeroKnownProofs(p, q, gA, A,
				x2.multiply(pwd).mod(q), signerId);
	...
	public String sessionKey(BigInteger gx4, BigInteger x2, BigInteger p,
			BigInteger q, BigInteger B, BigInteger pwd)
			throws NoSuchAlgorithmException {
		BigInteger k = getHash(gx4.modPow(x2.multiply(pwd).negate().mod(q), p)
	...
	/*
	 * class step2Result { private BigInteger gA; private BigInteger A; private
	 * BigInteger[] sigX2s;
	 * 
	 * public void setgA(BigInteger gA) { this.gA = gA; } public BigInteger
	 * getgA() { return gA; } public void setA(BigInteger A) { this.A = A; }
	 * public BigInteger getA() { return A; } public void setsigX2s(BigInteger[]
	 * sigX2s) { this.sigX2s = sigX2s; } public BigInteger[] getsigX2s() {
	 * return sigX2s; } }
	 */
